...
import { 
  View, Text, TextInput, TouchableOpacity, StyleSheet, FlatList, Alert, Keyboard 
} from 'react-native';
...
export default function MealsScreen() {
  ...
  // Ref para pular para o próximo input automaticamente
  const caloriesInputRef = useRef(null);

  const addMeal = () => {
    // Lógica atualizada conforme seu código
    if (mealName.trim() && calories && !isNaN(calories)) {
      const newMeal = {
        id: Date.now().toString(),
        name: mealName,
        calories: parseFloat(calories),
        // Ajustado para o formato pt-BR conforme seu código
      ...
      <View style={styles.card}>
        ...
        <View style={styles.formGroup}>
          ...
          <TextInput
            ...
            returnKeyType="next" // Botão do teclado vira "Próximo"
            onSubmitEditing={() => caloriesInputRef.current.focus()} // Pula para calorias
            blurOnSubmit={false}
          />
        </View>
        ...
        <View style={styles.formGroup}>
          ...
          <TextInput
            ...
            returnKeyType="done" // Botão do teclado vira "Concluído"
            onSubmitEditing={addMeal} // Chama a função ao dar Enter no celular
          />
        </View>
