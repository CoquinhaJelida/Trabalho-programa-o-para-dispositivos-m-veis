...
export default function ProfileScreen({ theme, onUpdateBg, onUpdateProfile }) {
  ...
  const loadAllData = () => {
    getProfile((data) => {
      if (data) {
        setName(data.name || '');
        // Força atualização do estado photo se existir no banco
        if (data.photo) {
            setPhoto(data.photo);
            if (onUpdateProfile) onUpdateProfile(data.photo); // Sincroniza header ao abrir
        }
        
        setAge(data.age || ''); setWeight(data.weight || ''); setHeight(data.height || '');
  ...
  // Salva dados (exceto a foto que tem função própria)
  useEffect(() => {
    if (name || age || weight) {
      // Mantém a foto atual no objeto salvo
  ...
  const updatePhoto = (uri) => {
    ...
    saveProfile({ 
        name, photo: uri, age, weight, height, calorieGoal, isStrict, targetWeight, gender, activityLevel, objective 
    });
  };
  ...
  const isBroken = isStrict && (todayCalories >= parseFloat(calorieGoal) * 1.5);
    ...
    <ScrollView contentContainerStyle={[styles.container, { backgroundColor: 'transparent' }]}>
      ...
      <View style={{ alignItems: 'center', marginBottom: 20 }}>
        <TouchableOpacity onPress={handleChangePhoto} style={[styles.profilePicContainer, { shadowColor: theme.text }]}>
          {photo ? (
            <Image source={{ uri: photo }} style={[styles.profilePic, { borderColor: theme.card }]} />
          ) : (
      ...
      <View style={cardStyle}><Text style={[styles.cardTitle, textStyle]}>Dados Pessoais</Text><Text style={[styles.label, subTextStyle]}>Nome</Text><TextInput style={inputStyle} value={name} onChangeText={setName} placeholder="Nome" placeholderTextColor={theme.textSub} /><View style={styles.row}><View style={styles.halfInput}><Text style={[styles.label, subTextStyle]}>Idade</Text><TextInput style={inputStyle} value={age} onChangeText={setAge} keyboardType="numeric" placeholderTextColor={theme.textSub} /></View><View style={styles.halfInput}><Text style={[styles.label, subTextStyle]}>Altura</Text><TextInput style={inputStyle} value={height} onChangeText={setHeight} keyboardType="numeric" placeholderTextColor={theme.textSub} /></View></View><View style={styles.row}><View style={styles.halfInput}><Text style={[styles.label, subTextStyle]}>Peso</Text><TextInput style={inputStyle} value={weight} onChangeText={setWeight} keyboardType="numeric" placeholderTextColor={theme.textSub} /></View><View style={styles.halfInput}><Text style={[styles.label, subTextStyle]}>Meta Peso</Text><TextInput style={[inputStyle, {borderColor:'#3b82f6'}]} value={targetWeight} onChangeText={setTargetWeight} keyboardType="numeric" placeholderTextColor={theme.textSub} /></View></View>{weightDiff && <Text style={{color:theme.textSub, fontSize:12, textAlign:'center', marginTop:-10, marginBottom:15}}>{getWeightFeedback()}</Text>}<View style={{marginTop:10, padding:15, backgroundColor: theme.isDark ? '#111827' : '#f0f9ff', borderRadius:12, borderWidth:1, borderColor: theme.border}}><Text style={{color: theme.primary, fontWeight:'bold', marginBottom:10}}>Calculadora TDEE</Text><View style={styles.selectRow}><TouchableOpacity style={[styles.selectBtn, gender==='male'&&{backgroundColor:theme.primary}]} onPress={()=>setGender('male')}><Text style={[styles.selectText, gender==='male'&&{color:'#fff'}]}>Homem</Text></TouchableOpacity><TouchableOpacity style={[styles.selectBtn, gender==='female'&&{backgroundColor:theme.primary}]} onPress={()=>setGender('female')}><Text style={[styles.selectText, gender==='female'&&{color:'#fff'}]}>Mulher</Text></TouchableOpacity></View><View style={{flexDirection:'row', flexWrap:'wrap', gap:5, marginBottom:10}}>{[{l:'Sedentário',v:1.2},{l:'Leve',v:1.375},{l:'Mod',v:1.55},{l:'Intenso',v:1.725}].map(i=>(<TouchableOpacity key={i.v} style={[styles.chip, activityLevel===i.v&&{backgroundColor:theme.primary, borderColor:theme.primary}]} onPress={()=>setActivityLevel(i.v)}><Text style={[styles.chipText, activityLevel===i.v&&{color:'#fff'}]}>{i.l}</Text></TouchableOpacity>))}</View><TouchableOpacity style={[styles.calcButton, {backgroundColor:theme.primary}]} onPress={handleAutoCalculate}><Feather name="cpu" size={20} color="#fff" /><Text style={{color:'#fff', fontWeight:'bold', marginLeft:8}}>Calcular Meta</Text></TouchableOpacity><View style={[styles.macroSuggestion, {backgroundColor: theme.inputBg}]}><Text style={[styles.macroTitle, subTextStyle]}>Sugestão:</Text><View style={styles.macroRow}><Text style={[styles.macroItem, textStyle]}>🥩 {macros.p}g P</Text><Text style={[styles.macroItem, textStyle]}>🥔 {macros.c}g C</Text><Text style={[styles.macroItem, textStyle]}>🥑 {macros.f}g G</Text></View></View></View><Text style={[styles.label, subTextStyle]}>Meta Kcal</Text><TextInput style={[inputStyle, {borderColor:theme.primary, textAlign:'center', fontWeight:'bold'}]} value={calorieGoal} onChangeText={setCalorieGoal} keyboardType="numeric" /><View style={styles.switchRow}><View style={{flex:1}}><Text style={[styles.switchTitle, textStyle]}>Modo Rígido</Text><Text style={[styles.switchDesc, subTextStyle]}>Meta como teto.</Text></View><Switch value={isStrict} onValueChange={setIsStrict} trackColor={{false:theme.inputBg, true:theme.primary}} thumbColor={"#fff"} /></View></View>
      ...
      <Text style={[styles.historyTitle, textStyle]}>Histórico</Text>{sortedDates.map(date => { const dayData = history[date]; const isExpanded = expandedDate === date; const dayTotal = (dayData.meals||[]).reduce((acc, curr) => acc + Number(curr.calories), 0); return (<View key={date} style={[styles.historyItem, {backgroundColor: theme.card}]}><TouchableOpacity style={styles.historyHeader} onPress={() => setExpandedDate(isExpanded ? null : date)}><View style={styles.dateBadge}><Text style={styles.dateText}>{formatDate(date)}</Text></View><View style={styles.headerInfo}><Text style={[styles.headerKcal, {color: theme.primary}]}>{Math.round(dayTotal)} kcal</Text><Text style={styles.headerWater}>💧 {dayData.water||0} ml</Text></View><Feather name={isExpanded ? "chevron-up" : "chevron-down"} size={20} color={theme.textSub} /></TouchableOpacity>{isExpanded && <View style={[styles.historyDetails, {backgroundColor: theme.inputBg, borderTopColor: theme.border}]}>{(dayData.meals||[]).map((m, i)=>(<Text key={i} style={[styles.mealName, textStyle]}>• {m.name}</Text>))}<TouchableOpacity style={styles.deleteDayButton} onPress={()=>handleDeleteDay(date)}><Text style={styles.deleteDayText}>Apagar</Text></TouchableOpacity></View>}</View>); })}
...
const styles = StyleSheet.create({
  ...
  profilePicContainer: { width: 100, height: 100, borderRadius: 50, elevation: 5, shadowOffset: {width:0, height:2}, shadowOpacity:0.2, shadowRadius:4, marginBottom: 5, backgroundColor: '#fff' }, // Adicionado BG branco por trás
