...
export default function WaterScreen({ onGainXP }) {
  ...
  const addWater = (amount) => {
    ...
    saveDailyLog(getTodayKey(), { water: newTotal, goal: goal }).then(() => {
      updateStreak();
  ...
  const handleAddCustom = () => {
    ...
    if (!amount || isNaN(amount) || amount <= 0) { Alert.alert("Erro", "Valor inválido."); return; }
    addWater(amount);
    setCustomAmount('');
    Keyboard.dismiss();
  };
  ...
  const resetDay = () => {
    setConsumed(0);
    saveDailyLog(getTodayKey(), { water: 0, goal: goal }).then(() => { updateStreak(); });
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {!hasProfile ? (
        <View style={styles.warningCard}><Text style={styles.warningText}>Configure seu peso no Perfil.</Text></View>
      ) : (
        <View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.customRow}>
              <TextInput style={styles.customInput} placeholder="Outra qtd (ml)" placeholderTextColor="#9ca3af" keyboardType="numeric" value={customAmount} onChangeText={setCustomAmount} />
          ...
          {streak > 0 && (
            <View style={styles.streakCard}>
              <View style={styles.streakIcon}><Feather name="zap" size={24} color="#f59e0b" /></View>
              <View style={{flex: 1}}><Text style={styles.streakTitle}>Sequência Incrível!</Text><Text style={styles.streakText}>Parabéns, você está há <Text style={{fontWeight: 'bold', color: '#f59e0b'}}>{streak} dias</Text> hidratado!</Text></View>
            </View>
          )}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  streakCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff7ed', padding: 16, borderRadius: 16, borderWidth: 1, borderColor: '#ffedd5', marginBottom: 20 },
  streakIcon: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#fef3c7', alignItems: 'center', justifyContent: 'center', marginRight: 15 },
